// Full ISO 3166-1 country list with dialling codes, for the checkout
// country select + WhatsApp dial-code dropdown. Sorted by name.
...
// Unique dial codes for the WhatsApp dropdown, labelled with a sample country.
// Sorted by numeric dial value so the list is easy to scan.
export const DIAL_OPTIONS = (() => {
  const seen = new Map();
  for (const c of COUNTRY_DATA) if (!seen.has(c.dial)) seen.set(c.dial, c.name);
  return [...seen.entries()]
    .map(([dial, name]) => ({ dial, label: `${dial}` }))
